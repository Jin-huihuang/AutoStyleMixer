...
class MixStyle2(nn.Module):
    ...
    def __init__(self, initial_value=0.5, T=0.3, eps=1e-6, lmda=None, num_features=None, domain_n=None, momentum=0.9, **kwargs):
        ...
        if not self.hparams['random']:
            ...
            self.beta = torch.distributions.Beta(0.3, 1)
        # print("* MixStyle params")
        # print(f"- p: {p}")
        # print(f"- alpha: {alpha}")
    ...
    def forward(self, x, activated=False):
        ...
        if self.hparams['EMA']:
            ...
            mu2, sig2 = new_statistics[0][perm].repeat(1, B//self.domain_n, 1, 1, 1).view(B, C, 1, 1), new_statistics[1][perm].repeat(1, B//self.domain_n, 1, 1, 1).view(B, C, 1, 1)
        x_normed = (x - mu) / sig
