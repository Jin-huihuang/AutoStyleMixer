...
def main(args, hparams):
    def objective(trial: optuna.trial.Trial):
        ...
        hparams['lr'] = trial.suggest_float('lr', 1e-5, 9e-5, step=1e-5)
        hparams['cls_w'] = trial.suggest_float('cls_w', 0, 1, step=0.01)
        # hparams['contrast_w'] = trial.suggest_float('contrast_w', 0.01, 1, log=True)
        hparams['Text'] = args.Text
        
        hparams['resnet_dropout'] = trial.suggest_float('resnet_dropout', 0, 0.5, step=0.1)
        # hparams['weight_decay'] = trial.suggest_float('weight_decay', 0, 1e-3, log=True)
        hparams['epsilon'] = trial.suggest_float('epsilon', 0, 0.1, step=0.001)
    ...
    study = optuna.create_study(study_name='Terra' ,direction='maximize', storage='sqlite:///db.sqlite3')
